...
async def create_routine(request: Request, db: SessionDep, user: AuthDep, body: RoutineCreate):
    return _get_service(db).create_routine(name=body.name, description=body.description, user_id=user.id)
...
async def get_routine(routine_id: int, request: Request, db: SessionDep, user: AuthDep):
    return await RoutineService(RoutineRepository(db), WorkoutRepository(db), _get_search_service()).get_routine(routine_id, user.id)
...
async def delete_routine(routine_id: int, request: Request, db: SessionDep, user: AuthDep):
    _get_service(db).delete_routine(routine_id, user.id)
...
async def remove_workout_from_routine(
    ...
    _get_service(db).remove_workout_from_routine(routine_workout_id, routine_id, user.id)
