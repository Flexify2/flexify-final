...
from .router_utils import get_search_service, require_user_id
...
async def create_routine(request: Request, db: SessionDep, user: AuthDep, body: RoutineCreate):
    return _get_service(db).create_routine(name=body.name, description=body.description, user_id=require_user_id(user))
...
async def get_routine(routine_id: int, request: Request, db: SessionDep, user: AuthDep):
    return await RoutineService(RoutineRepository(db), WorkoutRepository(db), get_search_service()).get_routine(routine_id, require_user_id(user))
...
async def update_routine(routine_id: int, request: Request, db: SessionDep, user: AuthDep, body: RoutineUpdate):
    ...
    return _get_service(db).update_routine(routine_id, body.name, body.description, require_user_id(user))
...
async def delete_routine(routine_id: int, request: Request, db: SessionDep, user: AuthDep):
    _get_service(db).delete_routine(routine_id, require_user_id(user))
...
async def remove_workout_from_routine(
    ...
    _get_service(db).remove_workout_from_routine(routine_workout_id, routine_id, require_user_id(user))
